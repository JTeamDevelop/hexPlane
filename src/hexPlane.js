...
hexPlaneMap.prototype.cellWithinX = function (cid,x) {
  var cell= {}, ncid = "", cA=[{n:[cid],o:[]}];
  cA.all=[cid];
  //loop through up to distance x
  for (var i = 1; i < x+1; i++) {
    //push a new array to cA representing cells at that distance, n for neighboors, o if they don't exist
    cA.push({n:[],o:[]});
    //loop  through cells at the previous distance
    for (var j = 0; j < cA[i-1].n.length; j++) {
      //load cell
      cell = this.cells[cA[i-1].n[j]];
      ...
      for (var k = 0; k < this.std_n.length; k++) {
        ...
        if(!cA.all.contains(ncid)){
          cA.all.push(ncid);
          //if it exists push it to n, otherwise o
          if(objExists(this.cells[ncid])){
            cA[i].n.push(ncid);
          }
          else {
            cA[i].o.push(ncid);
...
hexPlaneMap.prototype.cellNeighboors = function (cid) {
  //n will be neighboors, o will be open cells
  var neighboors = this.cellWithinX(cid,1);
  return neighboors[1];
}
